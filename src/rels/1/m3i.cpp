...
M3iImpl::M3iImpl(const il3int &list)
...
M3iImpl::M3iImpl(M3iImpl &&other) : ptr_(other.ptr_) { other.ptr_ = nullptr; }
...
std::ostream &operator<<(std::ostream &ostrm, const M3iImpl &m) {
  ostrm << "size: " << m.Size(0) << " " << m.Size(1) << " " << m.Size(2)
        << std::endl;
...
M3i::M3i(int d1, int d2, int d3, int fill_with) : impl_(d1, d2, d3) {
  this->Fill(fill_with);
}
...
int M3i::At(int i, int j, int k) const { return impl_.At(i, j, k); }
