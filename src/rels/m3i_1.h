...
#include <memory>
#include <mutex>
typedef std::initializer_list<std::initializer_list<std::initializer_list<int>>>
    il3int;
...
class M3iImpl {
 ...
 private:
  void Initialize(int***& arr, int d1, int d2,
                  int d3);  // заполнение arr значениями по умолчанию
...
class M3i {
 ...
 public:
  ...
  M3i(int d1, int d2, int d3, int fill_with);  // done
  // мой вариант конструктора от std::Initializer_list
